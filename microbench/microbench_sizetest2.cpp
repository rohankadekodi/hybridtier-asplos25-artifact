...
#include <tuple>
  ...
  #include "perf_lfu_sizeclass2.cpp"
#endif
...
typedef std::tuple<uint64_t,uint8_t> item_class_msg;
std::vector<item_class_msg> item_class_list;
...
void enable_lfu_microbench() {
  ...
  const char* enable_autonuma_cmd = "echo 0 > /proc/sys/vm/zone_reclaim_mode ; \
...
int main()
{
  ...
  // specify size classes. Here, all rows have size 8 pages (32KB)
  // size class format is: (start_addr, num_pages)
  uint8_t size_class = 8;
  for (int row = 0; row < NUM_ROWS; row++){
    // pass (&(arr[row*rowSize]), 8) to LFU thread
    item_class_list.push_back(std::make_tuple(reinterpret_cast<uintptr_t>(&(arr[row*rowSize])), size_class));
    //std::cout << std::hex << "sending " << &(arr[row*rowSize]) << ", " << std::dec << size_class << std::endl;
  }
  std::cout << "send size class done." << std::endl;
#endif
  ...
  for (long j = 0; j < 20; ++j) {
    ...
    if (j == 5) {
      ...
      std::cout << "Stopping the world to pagemap all items." << std::endl;
      // stop the world after LFU has finished migration
      for (long row = 0; row < rows; ++row) {
        //int count = hist[row];
        //if (count >= 7) {
          hot_rows.push_back(reinterpret_cast<uintptr_t>(&(arr[row*rowSize])));
          //std::cout << std::hex << reinterpret_cast<uintptr_t>(&(arr[row*rowSize])) << std::endl; // remove
        //}
      }

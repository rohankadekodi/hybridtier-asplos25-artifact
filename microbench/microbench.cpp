...
#define NUM_ROWS 2L*1024L*1024L
...
int main()
{
  ...
  std::normal_distribution<> d{1000000, 200000};
  ...
  // array of generated requests. Each element indicate which row to access.
  ...
  // If std dev = 100k, this means 27.2 reqs per row on average.
  ...
  for (long j = 0; j < 20; ++j) {
    std::cout << "Start access iteration " << j << std::endl;
    #pragma omp parallel for

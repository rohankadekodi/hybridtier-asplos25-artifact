...
std::unordered_set<uint64_t> sampled_pages;
...
void* perf_func(void*) {
    ...
    std::cout << "perf LFU. no tiering, just measure churn 2 (how do page hotness change over time?)" << std::endl;
    ...
    // Used to counter every X seconds
    std::chrono::steady_clock::time_point prev_clock = std::chrono::steady_clock::now();

    std::vector<uint64_t> tracking_hot_pages; 
    std::vector<uint64_t> tracking_cold_pages; 
    std::vector<int> tracking_hot_pages_hotness; 
    std::vector<int> tracking_cold_pages_hotness; 

    bool first_profile_interval = true;
              ...
              switch(ph->type) {
                case PERF_RECORD_SAMPLE:
                  ...
                  if (ps->addr != 0) { // sometimes the sample address is 0. Not sure why
                    ...
                    sampled_pages.insert(page_addr);

                    //if (sample_cnt % 50000000 == 0) {
                    std::chrono::steady_clock::time_point time_now = std::chrono::steady_clock::now();
                    double elapsed_time_ms = (double)std::chrono::duration_cast<std::chrono::milliseconds>(time_now - prev_clock).count();
                    if (elapsed_time_ms >= 20000) {
                      // Warning: spaghetti code ahead
                      prev_clock = std::chrono::steady_clock::now();
                      if (first_profile_interval) {
                        // For the first interval, pick X hot pages and X cold pages and track their hotness
                        uint16_t num_hot_pages_tracked = 0;
                        uint16_t num_cold_pages_tracked = 0;
                        // Find 10 hot pages
                        for (const auto& page: sampled_pages) {
                          int page_freq = lfu.frequency(page);
                          if (page_freq > 5) {
                            tracking_hot_pages.push_back(page);
                            num_hot_pages_tracked++;
                            if (num_hot_pages_tracked == 100) {
                              break;
                            }
                          }
                        }
                        ...
                        for (const auto& page: sampled_pages) {
                          int page_freq = lfu.frequency(page);
                          if (page_freq < 2) {
                            tracking_cold_pages.push_back(page);
                            num_cold_pages_tracked++;
                            if (num_cold_pages_tracked == 100) {
                              break;
                            }
                          }
                        }
                        first_profile_interval = false;
                        printf("tracking the folling pages \n");
                        for (const auto& page: tracking_hot_pages) {
                          std::cout << std::hex << page << ",";
                        }
                        for (const auto& page: tracking_cold_pages) {
                          std::cout << std::hex << page << ",";
                        }
                        std::cout << std::endl;
                        lfu.reset();
                      } else {
                        printf("printing new hotness\n");
                        // Not the first time profiling interval.
                        std::cout << std::dec << "tracker,";
                        for (const auto& page: tracking_hot_pages) {
                          int page_freq = lfu.frequency(page);
                          //tracking_hot_pages_hotness.push_back(page
                          std::cout << page_freq << ",";
                        }
                        for (const auto& page: tracking_cold_pages) {
                          int page_freq = lfu.frequency(page);
                ...
                case PERF_RECORD_LOST:
                  ...
                  if (num_perf_record_lost % 10000 == 0) {

...
std::unordered_set<uint64_t> promotion_candidates;
std::unordered_set<uint64_t> demotion_candidates;

uint64_t num_promo_demotion_candidates = 0;
  ...
  for(uint64_t curr_address = start_address; curr_address < end_address; curr_address += pagemap_read_batch_size_num_pages * PAGE_SIZE) {
    ...
    for(uint64_t ii = 0; ii < pagemap_read_bytes/sizeof(uint64_t); ii++) {
      ...
      if (pfn > 0 && pfn < 0x8080000) {
        ...
        // Profile the number of demotion candidates
        demotion_candidates.insert(virtual_page_addr & ~(0xFFF));
...
void* perf_func(void*) {
    ...
    std::cout << "perf LFU tierng. promotion demotion candidate stats" << std::endl;
    ...
    frequency_sketch<uint64_t> momentum(NUM_LFU_ENTRIES/100, 1100000);
              ...
              switch(ph->type) {
                case PERF_RECORD_SAMPLE:
                  ...
                  if (ps->addr != 0) { // sometimes the sample address is 0. Not sure why
                    ...
                    // Profile the number of promotion/demotion candidates
                    promotion_candidates.insert(page_addr);
                    ...
                    if (move_page_ret == 0) { 
                      pages_migrated = pages_migrated + num_pages_to_migrate;
                    } else {
                    ...
                    //if (perf_sample_freq == perf_sample_freq_list[0]){
                    //  // Only sleep at lowest sampling frequency
                    //  if (check_sleep()) {
                    //    // If we are waking up from periodic sleep, restart perf sampling
                    ...
                    if (sample_cnt % 500000 == 0) {
                      for (const auto& element: promotion_candidates) {
                        if (demotion_candidates.find(element) != demotion_candidates.end()) {
                          num_promo_demotion_candidates++;
                        }
                      }
                      printf("promo/demote candidate stat: %lu, %lu, %lu \n", promotion_candidates.size(), demotion_candidates.size(), num_promo_demotion_candidates);
                      num_promo_demotion_candidates = 0;
                    }


                    //uint32_t new_sample_freq = check_fast_mem_hit_ratio(perf_sample_freq);
                    //if (perf_sample_freq == perf_sample_freq_list[0] && new_sample_freq == perf_sample_freq_list[0]) {
                    //  // If already at lowest sampling frequency and fast memory hit ratio is stable,
                    //  // we do not have many more hot pages to promote. 
                    //  std::cout << "[INFO] Fast memory hit ratio stable. Go to monitor mode. " << std::endl;
                    ...
                    //  goto perf_sampling_start;
                    //}
                    //if (new_sample_freq > 0 && new_sample_freq != perf_sample_freq) {
                    //  // Reconfigure perf counters with new sampling frequency.
                    //  perf_sample_freq = new_sample_freq;
                    //  // First close existing perf counters. 
                    //  close_perf();
                    //  // Reset the fast memory hit ratio history. This is because changing the sampling frequency will 
                    //  // effect the fast memory hit ratio. So we do not want the fast mem hit ratio from another sampling freq
                    //  // to affect the fast mem hit ratio of the current samplking freq.
                    //  fast_mem_hit_ratio_window.clear();
                    //  // Go to the beginning, which will create the new perf counters with the updated sampling frequency.
                    //  goto perf_sampling_start;
                    //}
                    ...
                    */
                    ...
                    num_pages_promoted_history += num_pages_to_migrate;
